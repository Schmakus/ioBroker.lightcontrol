 ...
 */
async function getAstroNameTs(astroName, offsetMinutes = 0) {
	try {
		let ts = adapter.x.mSuncalc.getTimes(
			new Date(),
			adapter.x.systemConfig.latitude,
			adapter.x.systemConfig.longitude,
		)[astroName];
		if (!ts || ts.getTime().toString() === "NaN") {
			...
			if (adapter.config.fixNightNightEnd && ["night", "nightEnd"].includes(astroName)) {
				...
				switch (astroName) {
					case "night":
						adapter.log.debug(
							`[getAstroNameTs] No time found for [${astroName}], so we set 00:00 as fallback per adapter config.`,
						);
						return midnightTs; // midnight - 00:00
					case "nightEnd":
						adapter.log.debug(
							`[getAstroNameTs] No time found for [${astroName}], so we set 02:00 as fallback per adapter config.`,
						);
