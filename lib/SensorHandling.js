 ...
 * @param adapter
*/
async function RefreshGenericLuxValues(adapter) { // Used by Init - refreshing ALL Groups using the generic Luxsensor with new value
	try {
		...
		for (const Group in LightGroups) {
			...
			if (LightGroups[Group].LuxSensor !== "" && LightGroups[Group].LuxSensor == adapter.GlobalSettings.GlobalLuxSensor) {
		...
		adapter.log.warn(`SummarizeSensors => ${e}`);
	}
}
