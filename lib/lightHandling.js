...
async function SetBrightness(adapter, Group, Brightness) {
	try {
		adapter.log.debug(`Reaching SetBrightness, Group="${Group}" Brightness="${Brightness}`);
...
async function AdaptiveBri(adapter, Group) {
	try {
		const LightGroups = adapter.LightGroups;
		adapter.log.debug(`Reaching AdaptiveBri for Group="${Group}" actual Lux="${LightGroups[Group].actualLux}" generic lux="${adapter.ActualGenericLux}`);
		...
		if (LightGroups[Group].adaptiveBri) {
			...
			if (LightGroups[Group].actualLux === 0) {
				...
				TempBri = adapter.GlobalSettings.minBri;
				...
				if (TempBri < adapter.GlobalSettings.minBri) TempBri = adapter.GlobalSettings.minBri;
...
async function setDeviceBri(adapter,Group, Brightness) {
	try {
		...
		for (const Light in LightGroups[Group].lights) {
			if (LightGroups[Group].lights[Light].bri.oid !== "") {
				...
				if (LightGroups[Group].lights[Light].power.oid !== ""){
					...
					await adapter.setForeignStateAsync(LightGroups[Group].lights[Light].bri.oid, Math.round(Brightness / 100 * LightGroups[Group].lights[Light].bri.defaultVal), false);
		...
		adapter.log.error(`setDeviceBri => ${e}`);
	}
}
...
async function SetCt(adapter, Group, ct = adapter.LightGroups[Group].ct) {
	try {
		...
		adapter.log.debug(`Reaching SetCt, Group="${Group}" Ct="${LightGroups[Group].ct}"`);
		...
		if (LightGroups[Group].power) {
			...
			for (const Light in LightGroups[Group].lights) {
				if (LightGroups[Group].lights[Light].ct.oid !== "") {
					...
					const TempCt = LightGroups[Group].lights[Light].ct.minVal < 1000 ? await ConvertKelvin(adapter, LightGroups[Group].lights[Light].ct.minVal, LightGroups[Group].lights[Light].ct.maxVal, ct) : ct;
...
async function AdaptiveCt(adapter) {
	try {
		...
		const minCt = adapter.GlobalSettingsminCt;
		const maxCt = adapter.GlobalSettingsmaxCt;
		...
		adapter.log.debug(`AdaptiveCt => adaptiveCtLinear="${adaptiveCtLinear}" adaptiveCtSolar="${adaptiveCtSolar}" adaptiveCtSolarInterpolated="${adaptiveCtSolarInterpolated}" adaptiveCtTimed="${adaptiveCtTimed}`);
		...
		adapter.log.error(`AdaptiveCt => ${e}`);
	}
}
...
async function SetWhiteSubstituteColor(adapter, Group) {
	try {
		...
		adapter.log.debug(`Reaching WhiteSubstituteColor for Group="${Group}" = "${LightGroups[Group].description}" LightGroups[Group].power="${LightGroups[Group].power}" LightGroups[Group].color="${LightGroups[Group].color}`);
			...
			if (LightGroups[Group].ct < ((maxCt - minCt) / 4 + minCt) || LightGroups[Group].ct > ((maxCt - minCt) / 4 * 3 + minCt)) { //Ct Regelbereich vierteln, erstes viertel ist ww, 2tes und drittes wieder kw, das letzte ww
				...
				for (const Light in LightGroups[Group].lights) {
					if (LightGroups[Group].lights[Light].ct.oid === "" && LightGroups[Group].lights[Light].color.oid !== "" && LightGroups[Group].lights[Light].color.warmWhiteColor !== "" && LightGroups[Group].lights[Light].color.dayLightColor !== "") {
						...
						await adapter.setForeignStateAsync(LightGroups[Group].lights[Light].color.oid, LightGroups[Group].lights[Light].color.warmWhiteColor, false);
			...
			//  log("Kaltweiss")
				for (const Light in LightGroups[Group].lights) {
					if (LightGroups[Group].lights[Light].ct.oid === "" && LightGroups[Group].lights[Light].color.oid !== "" && LightGroups[Group].lights[Light].color.warmWhiteColor !== "" && LightGroups[Group].lights[Light].color.dayLightColor !== "") {
						...
						await adapter.setForeignStateAsync(LightGroups[Group].lights[Light].color.oid, LightGroups[Group].lights[Light].color.dayLightColor, false);
...
async function SetColorMode(adapter, Group) {
	try {
		...
		if (LightGroups[Group].power) {
			...
			for (const Light in LightGroups[Group].lights) { //Alle Lampen der Gruppe durchgehen
				if (LightGroups[Group].lights[Light].modeswitch.oid !== "") { //Prüfen ob Datenpunkt für Colormode vorhanden
					...
					if (LightGroups[Group].color.toUpperCase() == "#FFFFFF") { //bei Farbe weiss
						...
						await adapter.setForeignStateAsync(LightGroups[Group].lights[Light].modeswitch.oid, LightGroups[Group].lights[Light].modeswitch.whiteModeVal, false);
						adapter.log.info(`Device="${LightGroups[Group].lights[Light].modeswitch.oid}" Val="${LightGroups[Group].lights[Light].modeswitch.whiteModeVal}`);

					} else { //Bei allen anderen Farben

						await adapter.setForeignStateAsync(LightGroups[Group].lights[Light].modeswitch.oid, LightGroups[Group].lights[Light].modeswitch.colorModeVal, false);
...
async function SetColor(adapter, Group, Color) {
	try {
		const LightGroups = adapter.LightGroups;
		adapter.log.debug(`Reaching SetColor for Group="${Group}" power="${LightGroups[Group].power}" Color="${Color}"`);
				...
				if (LightGroups[Group].lights[Light].color.oid != "") { //Prüfen ob Datenpunkt für Color vorhanden
					...
					switch (LightGroups[Group].lights[Light].color.type) {
						case "hex": //Keine Konvertierung nötig
							await adapter.setForeignStateAsync(LightGroups[Group].lights[Light].color.oid, Color, false);
							break;
						case "rgb":
							await adapter.setForeignStateAsync(LightGroups[Group].lights[Light].color.oid, rgbTemp, false);
							break;
						case "xy":
							await adapter.setForeignStateAsync(LightGroups[Group].lights[Light].color.oid, await colorConv.ConvertRgbToXy(rgbTemp), false);
							break;
						default:
							adapter.log.warn(`SetColor: Unknown colortype in Group="${Group}", please specify!`);
					}
				}
			}
...
*/
async function PowerOnAftercare(adapter, Group, bri = adapter.LightGroups[Group].bri, ct = adapter.LightGroups[Group].ct, color = adapter.LightGroups[Group].color) {
	try {
		const LightGroups = adapter.LightGroups;
		...
		adapter.log.error(`PowerOnAftercare => ${e}`);
	}
}
