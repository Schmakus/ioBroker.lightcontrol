...
async function SetWhiteSubstituteColor(adapter, Group) {
	...
	try {
		...
		const promisesKelvinWithColor = LightGroups[Group].lights
			.filter(
				(Light) =>
					!Light?.ct?.oid &&
					Light?.color?.oid &&
		...
		await Promise.all([promisesWarmWhiteDayLight, promisesKelvinWithColor]);
	} catch (error) {
					...
					switch (Light.color.colorType) {
						...
						case "hue": {
							if (Light.bri?.oid && Light.sat?.oid) {
								const colorValue = await colorConv.ConcertHexToHue(Color);
								await Promise.all([
									adapter.setForeignStateAsync(Light.color.oid, colorValue.hue, false),
									adapter.setForeignStateAsync(Light.sat.oid, colorValue.saturation, false),
									adapter.setForeignStateAsync(Light.bri.oid, colorValue.brightness, false),
