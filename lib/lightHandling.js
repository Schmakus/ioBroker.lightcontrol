		...
		await Promise.all(
			LightGroups[Group].lights.map(async (Light) => {
				...
				if ((LightGroups[Group].power || ct?.sendCt) && ct?.oid) {
					...
					const minKelvin = ct?.minValKelvin ?? 2700;
					const maxKelvin = ct?.maxValKelvin ?? 6500;		
					const CtConversion = ct?.CtConversion ?? "default";
					const value = await KelvinToRange(adapter, outMinCt, outMaxCt, minKelvin, maxKelvin, ctValue, CtConversion);
					adapter.writeLog(`[ SetCt ] Calculated ct-value: ${value}`);
					if (value === -3) {
						adapter.writeLog(
							`[ KelvinToRange ] Not able to set Color-Temperature for Light = "${oid}" in Group = "${Group}". No conversion defined!!`,
							"warn",
						);
					}
					if (value >= 0) await adapter.setForeignStateAsync(oid, value, false);
				}
			}),
		);
 ...
 */
async function KelvinToRange(adapter, outMinCt, outMaxCt, minKelvin, maxKelvin, kelvin, CtConversion = "default") {
	try {
		...
		minKelvin = minKelvin < minCt ? minCt : minKelvin;
		maxKelvin = maxKelvin < maxCt ? maxCt : maxKelvin;
		
		kelvin = kelvin < minKelvin ? minKelvin : (kelvin > maxKelvin ? maxKelvin : kelvin); // constrain kelvin to min and max kelvin
		...
		switch(CtConversion) {
			case "default":
				return -2;
			case "warmToCool":
				rangeValue = ((kelvin - minKelvin) / (maxKelvin - minKelvin)) * (outMaxCt - outMinCt) + outMinCt;
				break;
			case "coolToWarm":
				rangeValue = ((maxKelvin - kelvin) / (maxKelvin - minKelvin)) * (outMaxCt - outMinCt) + outMinCt;
				break;
			case "mired":
