...
const { compareTime, getDateObject, convertTime, getAstroDate } = require("./helper");
	...
	try {
		...
		await Promise.all(
			LightGroups[Group].lights.map(async (Light) => {
				...
				if ((LightGroups[Group].power || color?.sendCt) && color?.oid && color?.setCtwithColor) {
			...
			switch (LightGroups[Group].adaptiveCtMode) {
				...
				case "timed":
					if (LightGroups[Group].adaptiveCt?.enabled && LightGroups[Group].ct !== adaptiveCtTimed) {
						morningTime = (await getDateObject(LightGroups[Group].adaptiveCt?.adaptiveCtTime)).getTime();
						...
						adapter.writeLog(
							`[ AdaptiveCt // timed ] morningTime="${LightGroups[Group].adaptiveCt?.adaptiveCtTime}" => "${morningTime}", ActualTime="${ActualTime}", sunset="${sunset}", adativeCtTimed="${adaptiveCtTimed}"`,
						);
				...
				case "timedInterpolated":
					if (
						...
						adapter.writeLog(
							`[ AdaptiveCt // timedInterpolated ] morningTime="${LightGroups[Group].adaptiveCt?.adaptiveCtTime}" => "${morningTime}", ActualTime="${ActualTime}", sunset="${sunset}", adativeCtTimed="${adaptiveCtTimedInterpolated}"`,
						);
...
async function SetWhiteSubstituteColor(adapter, Group) {
	...
	try {
		...
		const promisesKelvinWithHUE = LightGroups[Group].lights
			...
			.map(async (Light) => {
				const colorValue = await colorConv.ConvertKelvinToHue(adapter.ct);
				await Promise.all([
					adapter.setForeignStateAsync(Light.color.oid, colorValue.hue, false),
					adapter.setForeignStateAsync(Light.sat.oid, colorValue.saturation, false),
					adapter.setForeignStateAsync(Light.bri.oid, colorValue.brightness, false),
				]);
			});
		...
		const promisesKelvinWithRGB = LightGroups[Group].lights
			...
			.map(async (Light) => {
				const colorValue = await colorConv.convertKelvinToRGB(adapter.ct);
				await adapter.setForeignStateAsync(Light.color.oid, colorValue, false);
			});
		...
		const promisesKelvinWithXY = LightGroups[Group].lights
			...
			.map(async (Light) => {
				const rgb = await colorConv.convertKelvinToRGB(adapter.ct);
...
async function SetTt(adapter, Group, RampTime, prop) {
	...
	try {
		...
		adapter.writeLog(`[ SetTt ] Reaching for Group="${Group}", TransitionTime="${RampTime}ms"`);
		...
		const promises = LightGroups[Group].lights
			...
			.map(async (Light) => {
				...
				await Promise.all([
					adapter.setForeignStateAsync(Light.tt.oid, { val: tt, ack: false }),
					adapter.writeLog(`[ SetTt ] Set ${Light.description} (${Light.tt.oid}) to: ${tt}${Light.tt.unit}`),
				]);
			});
		...
		adapter.errorHandling(error, "SetTt", `RampTime: ${RampTime}, Prop: ${prop}`);
	}
}
