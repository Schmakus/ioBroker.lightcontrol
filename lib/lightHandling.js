		...
		for (const Light of LightGroups[Group].lights) {
			if (Light?.bri?.oid) {
				if (!Light.bri.sendBri) {
					...
					adapter.writeLog(
						`[ setDeviceBri ] Switching with Brightness is activated. No min. Brightness defined. Actual Brightness = "${Brightness}"`,
					);
				...
				await adapter.setForeignStateAsync(
					Light.bri.oid,
					// @ts-ignore
					Math.round((Brightness / 100) * Light.bri.defaultBri),
					false,
				);
			}
		}
