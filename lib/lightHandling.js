			...
			switch (LightGroups[Group].adaptiveCtMode) {
				...
				case "timed":
					if (LightGroups[Group].adaptiveCt && LightGroups[Group].ct !== adaptiveCtTimed) {
						...
						adapter.writeLog(
							`[ AdaptiveCt // timed ] morningTime="${LightGroups[Group].adaptiveCtTime}" => "${morningTime}", ActualTime="${ActualTime}", sunMinutesDay="${sunMinutesDay}", RangPerMinute="${RangePerMinute}"`,
						);
						...
						if (ActualTime >= morningTime && ActualTime <= sunrise) {
							const interpolatedValue =
								maxCt + ((minCt - maxCt) * (ActualTime - morningTime)) / (sunrise - morningTime);
							adapter.writeLog(`Interpolated Value: ${interpolatedValue}`);
						}
				...
				case "timedInterpolated":
					if (LightGroups[Group].adaptiveCt && LightGroups[Group].ct !== adaptiveCtTimedInterpolated) {
						...
						if (ActualTime >= morningTime && ActualTime <= sunrise) {
							const timeFraction = (ActualTime - morningTime) / (sunrise - morningTime);
							const radians = 2 * Math.PI * timeFraction;
							adaptiveCtTimedInterpolated =
								morningTime + (maxCt - minCt) * (0.5 * (1 + Math.sin(radians)));
						}
