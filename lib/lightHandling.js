		...
		await Promise.all(
			LightGroups[Group].lights.map(async (Light) => {
				...
				if ((LightGroups[Group].power || ct?.sendCt) && ct?.oid) {
					...
					const ctConversation = ct?.ctConversation ?? "default";
 ...
 * @param {string} ctConversation	switch if lower ct-value is cold
 * @returns {Promise<number>} return the Ct-Value of the target state
 */
async function KelvinToRange(adapter, outMinCt, outMaxCt, minKelvin, maxKelvin, kelvin, ctConversation) {
	try {
		...
		kelvin = Math.round(Math.min(Math.max(kelvin, minKelvin), maxKelvin));

		switch (ctConversation) {
			case "default":
				return -2;
			case "warmToCold":
