...
async function Init(adapter) {
	adapter.writeLog(`Init is starting...`);
...
async function GlobalLuxHandling(adapter) {
	if (adapter.Settings.GlobalLuxSensor && adapter.Settings.GlobalLuxSensor !== "") {
		const _actualGenericLux = await adapter
			.getForeignStateAsync(adapter.Settings.GlobalLuxSensor)
		...
		if (_actualGenericLux || _actualGenericLux === 0) {
			...
			adapter.log.warn(
				`GlobalLuxHandling => ObjectID "${adapter.Settings.GlobalLuxSensor}" liefert NULL, undefined!!`,
			);
		}
	}
}
...
async function GlobalPresenceHandling(adapter) {
	try {
		if (adapter.Settings.PresenceCountDp !== "") {
			adapter.log.debug(`GlobalPresenceHandling: PresenceCounteDp=${adapter.Settings.PresenceCountDp}`);
		...
		adapter.log.error(
			`GlobalPresenceHandling: Object-ID "adapter.Settings.PresenceCounterDp" not exits. Please check your config! (${e})`,
		);
	}

	try {
		if (adapter.Settings.IsPresenceDp !== "") {
			adapter.log.debug(`GlobalPresenceHandling: IsPresenceDp=${adapter.Settings.IsPresenceDp}`);
		...
		adapter.log.error(
			`GlobalPresenceHandling: Object-ID "adapter.Settings.IsPresenceDp" not exits. Please check your config! (${e})`,
		);
	}
...
async function StatesCreate(adapter) {
	...
	for (const Group in LightGroups) {
		...
		try {
			...
			adapter.writeLog(`CreateStates => ${e}`, "error");
		}
	}
	...
	for (const prop1 in DeviceAllTemplate) {
		try {
			...
			if (prop1 === "power") {
				adapter.writeLog(`CreateDevice => Group="All", Prop1="${prop1}", powerNewVal="${state.val}"`);
			...
			if (state != null) {
				...
				adapter.writeLog(`CreateDevice => State: "${dp}" is NULL or undefined!`, "warn");
			}

			// Subscribe on state changes if writable
			common.write && (await adapter.subscribeStatesAsync(dp));
		} catch (e) {
			adapter.writeLog(`CreateDevice => Error by creating All channel: ${prop1}. Error: ${e}`, "error");
		}
	}
...
async function CreateStates(adapter, dp, common) {
	try {
		...
		if (!obj) {
			...
			adapter.writeLog(`CreateStates => State: ${dp} was created.`);
		} else {
			if (JSON.stringify(obj.common) !== JSON.stringify(common)) {
				await adapter.extendObjectAsync(dp, {
					common: common,
				});
				adapter.writeLog(`CreateStates => State: "${dp}" was extended`);
			}
		}
	} catch (e) {
		adapter.writeLog(`CreateStates => Error by crating state: ${e}`, "error");
	}
}
/**
...
async function CreateChannel(adapter, dp, name) {
	try {
		...
		adapter.writeLog(`Error by crating channel: ${e}`, "error");
	}
}
/**
