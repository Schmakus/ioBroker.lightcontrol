...
async function Init(adapter) {
	...
	adapter.writeLog(`Init => LightGroups Memory: ${JSON.stringify(adapter.LightGroups)}`);
...
async function CreateStates(adapter, dp, common, foreign) {
	try {
		...
		if (!obj) {
			...
			await (foreign ? adapter.setForeignObjectAsync(dp, obj) : adapter.setObjectAsync(dp, obj));
