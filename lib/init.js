...
async function Init(adapter) {
	...
	await InitCustomStates(adapter);
 ...
 * @param adapter Adapter-Class
 */
async function InitCustomStates(adapter) {
	try {
		// Get all objects with custom configuration items
		const customStateArray = await adapter.getObjectViewAsync("system", "custom", {});
		adapter.writeLog(`InitCustomStates => All states with custom items : ${JSON.stringify(customStateArray)}`);
		...
		if (customStateArray && customStateArray.rows) {
			...
			for (const index in customStateArray.rows) {
				if (customStateArray.rows[index].value) {
					...
					if (customStateArray.rows[index].value[adapter.namespace]) {
						adapter.writeLog(`InitCustomStates => LightControl configuration found`);

						// Simplify stateID
						const stateID = customStateArray.rows[index].id;

						// Check if custom object is enabled for LightControl
						if (customStateArray.rows[index].value[adapter.namespace].enabled) {
							// Prepare array in constructor for further processing
							adapter.activeStates[stateID] = {};
							adapter.writeLog(`InitCustomStates => LightControl enabled state found ${stateID}`);
						} else {
							adapter.writeLog(
		...
		const totalEnabledStates = Object.keys(adapter.activeStates).length;
		let totalInitiatedStates = 0;
		let totalFailedStates = 0;
		adapter.writeLog(`Found ${totalEnabledStates} LightControl enabled states`, "info");

		// Initialize all discovered states
		let count = 1;
		for (const stateID in adapter.activeStates) {
			adapter.writeLog(`InitCustomStates => Initialising (${count} of ${totalEnabledStates}) "${stateID}"`);
			//await adapter.buildStateDetailsArray(stateID);
			adapter.writeLog(`InitCustomStates = > hier muss das LightGroups Object erstellt werden`);
			if (adapter.activeStates[stateID]) {
				await adapter.initialize(stateID);
				totalInitiatedStates = totalInitiatedStates + 1;
				adapter.writeLog(`Initialization of ${stateID} successfully`, "info");
			} else {
				adapter.writeLog(`Initialization of ${stateID} failed, check warn messages !`, "error");
