...
const { TestTemplateLamps, TestTemplateMotionSensors, TestTemplateLuxSensors, TestTemplatePresence } = require(`./testTemplates`);
...
async function GlobalLuxHandling(adapter) {
	if (adapter.GlobalSettings.GlobalLuxSensor !== "") {
		const _actualGenericLux = await adapter.getForeignStateAsync(adapter.GlobalSettings.GlobalLuxSensor).catch((e) => adapter.log.error("GlobalLuxHandling => " + e));
...
async function DoAllTheMotionSensorThings(adapter, Group) {
	...
	for (const sensorCount in adapter.LightGroups[Group].sensors) {
		...
		const _motionState = await adapter.getForeignStateAsync(LightGroups[Group].sensors[sensorCount].oid).catch((e) => adapter.log.error(e));
		if (_motionState) {
			LightGroups[Group].sensors[sensorCount].isMotion = (_motionState.val == LightGroups[Group].sensors[sensorCount].motionVal) ? true : false;
			adapter.log.debug(`DoAllTheMotionSensorThings => Group="${Group}" SensorID="${LightGroups[Group].sensors[sensorCount].oid}" MotionVal="${LightGroups[Group].sensors[sensorCount].isMotion}"`);
			...
			adapter.log.debug(`DoAllTheMotionSensorThings => Group="${Group}" ${LightGroups[Group].sensors[sensorCount].oid} has no data, skipping subscribe`);
		}
	}
}
...
async function DoAllTheLuxSensorThings(adapter, Group) {
	...
	if ((LightGroups[Group].LuxSensor === "" || LightGroups[Group].LuxSensor === undefined) && adapter.GlobalSettings.GlobalLuxSensor !== "") {
	...
	if (LightGroups[Group].LuxSensor !== "" || LightGroups[Group].LuxSensor !== undefined) {
		adapter.log.debug(`DoAllTheLuxSensorThings => LightGroups["${Group}"].LuxSensor="${LightGroups[Group].LuxSensor}"`);
		...
		if (LightGroups[Group].LuxSensor === adapter.GlobalSettings.GlobalLuxSensor) {
			...
			adapter.log.debug(`DoAllTheLuxSensorThings => Group "${Group}" using generic luxsensor, value is: ${LightGroups[Group].actualLux}`);
		} else {
			// Read individual lux value for group
			const _actualLux = await adapter.getForeignStateAsync(LightGroups[Group].LuxSensor).catch((e) => adapter.log.error("DoAllTheLuxSensorThings => " + e));
			if(_actualLux) {
				LightGroups[Group].actualLux = _actualLux.val;
				adapter.log.debug(`DoAllTheLuxSensorThings => Group="${Group}" using individual luxsensor "${LightGroups[Group].LuxSensor}", value is: ${LightGroups[Group].actualLux}`);
				await adapter.subscribeForeignStatesAsync(LightGroups[Group].LuxSensor);
				adapter.LuxSensors.push(LightGroups[Group].LuxSensor);
			} else {
				adapter.log.debug(`DoAllTheLuxSensorThings => No Luxsensor for Group="${Group}" defined, skip handling`);
			}
		}
	}
}
...
async function GlobalPresenceHandling(adapter) {
	try {
		if (adapter.GlobalSettings.PresenceCountDp !== "") {
			...
			adapter.ActualPresenceCount.newVal = (await adapter.getForeignStateAsync(adapter.GlobalSettings.PresenceCountDp)).val;
			...
			if (typeof adapter.ActualPresenceCount.newVal === "number") {
				...
				adapter.log.warn(`GlobalPresenceHandling: PresenceCounterDp=${adapter.GlobalSettings.PresenceCountDp} is not type="number"!`);
			}

		}
	} catch(e) {
		adapter.log.error(`GlobalPresenceHandling: Object-ID "adapter.GlobalSettings.PresenceCounterDp" not exits. Please check your config! (${e})`);
	}
...
=======
			//adapter.ActualPresence = (await adapter.getForeignStateAsync(adapter.GlobalSettings.IsPresenceDp)).val;
			const _ActualPresence = await adapter.getForeignStateAsync("0_userdata.0.lightcontrol_DEV.Presence.isPresenceDp");
			adapter.log.debug(_ActualPresence);
>>>>>>> main
...
<<<<<<< HEAD
		adapter.log.error(`GlobalPresenceHandling: Object-ID "adapter.GlobalSettings.IsPresenceDp" not exits. Please check your config! (${e})`);
=======
		adapter.log.error(`GlobalPresenceHandling (isPresenceDp): ${e}`);
>>>>>>> main
	}
		...
		try {
			for(const prop1 in DeviceTemplate) {
				...
				if (typeof DeviceTemplate[prop1].name == "undefined") {
					...
					if (prop1 === "power") {
						adapter.log.debug(`CreateStates => Group="${Group}", Prop1="${prop1}", powerNewVal="${state.val}"`);
...
async function CreateStates(adapter, dp, common) {
	try {
		...
		if (obj != null) {
			...
			if (obj.common.role != common.role
