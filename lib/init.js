	...
	try {
		...
		if (luxSensor) {
			if (luxSensor === adapter.Settings.GlobalLuxSensor) {
				...
				adapter.writeLog(`[ DoAllTheLuxSensorThings ] Group "${Group}" using global luxsensor.`);
			} else {
				const individualLux = await adapter.getForeignStateAsync(luxSensor);

				if (individualLux !== null) {
					const individualLuxVal = individualLux.val;

					if (typeof individualLuxVal === "number" && individualLuxVal.toString().length !== 0) {
						LightGroups[Group].actualLux = individualLuxVal;
...
async function CreateStates(adapter, dp, common, foreign) {
	try {
		...
		if (!obj) {
			...
			if (!foreign) {
				await adapter.setObjectNotExistsAsync(dp, stateObject);
			} else {
				await adapter.setForeignObjectNotExistsAsync(dp, stateObject);
			}
			...
			if (JSON.stringify(obj.common) !== JSON.stringify(common) || !("native" in obj)) {
				const stateObject = {
					type: "state",
					common: common,
					native: "native" in obj ? obj.native : {},
				};
				if (!foreign) {
					await adapter.setObjectAsync(dp, stateObject);
				} else {
					await adapter.setForeignObjectAsync(dp, stateObject);
				}
			}
		}
	} catch (error) {
