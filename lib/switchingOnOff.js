...
async function SimpleGroupPowerOnOff(adapter, Group, OnOff) {
	try {
		...
		if (OnOff) {
			...
			adapter.writeLog(`SimpleGroupPowerOnOff => Normales anschalten ohne Ramping für Group="${Group}"`);
			...
			adapter.writeLog(`SimpleGroupPowerOnOff => Normales ausschalten ohne Ramping für Group="${Group}"`);
