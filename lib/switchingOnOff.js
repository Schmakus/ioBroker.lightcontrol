	...
	try {
		...
		if (OnOff && !LightGroups[Group].rampOn.enabled) {
			...
			if (LightGroups[Group].rampOn.enabled) {
				...
				await SetBrightness(adapter, Group, 2);
			}
		...
		if (OnOff && LightGroups[Group].rampOn.enabled && LightGroups[Group].rampOn.switchOutletsLast) {
			...
			*/
			...
			try {
				const [rampResult, clearResult] = await Promise.all([
					DeviceSwitchForRamping(adapter, Group, OnOff),
					clearRampOnIntervals(adapter, Group),
				]);
				adapter.writeLog(`[ GroupPowerOnOff ] Ramp result: ${rampResult}, Clear result: ${clearResult}`);
			} catch (error) {
				adapter.errorHandling(
					error,
					"GroupPowerOnOff / Promise.all at Anschalten mit Ramping und einfache Lampen zuerst",
				);
			}
			...
			adapter.RampOnIntervalObject[Group] = setInterval(async function () {
				...
				// Helligkeit erhöhen
				await SetBrightness(adapter, Group, Math.round(RampSteps * LoopCount * (LightGroups[Group].bri / 100)));
			...
			*/
			...
			try {
				const [switchResult, rampResult, clearResult] = await Promise.all([
					DeviceSwitch(adapter, Group, OnOff),
					DeviceSwitchForRamping(adapter, Group, OnOff),
					clearRampOnIntervals(adapter, Group),
				]);
				adapter.writeLog(
					`[ GroupPowerOnOff ] Switch result: ${switchResult}, Ramp result: ${rampResult}, Clear result: ${clearResult}`,
				);
			} catch (error) {
				adapter.errorHandling(
					error,
				...
				if (LoopCount >= RampSteps) {
					...
					*/
					try {
						...
						adapter.writeLog(
							`[ GroupPowerOnOff ] Ramp result: ${rampResult}, Clear result: ${clearResult}`,
						);
					} catch (error) {
						adapter.errorHandling(
							error,
