...
async function SimpleGroupPowerOnOff(adapter, Group, OnOff) {
	try {
		...
		adapter.writeLog(
			`[ v${adapter.version} SimpleGroupPowerOnOff ] error: ${e.message} // stack: ${e.stack}`,
			"error",
		);
	}
}
...
async function GroupPowerOnOff(adapter, Group, OnOff) {
	try {
		...
		if (OnOff && LightGroups[Group].rampOn.enabled && LightGroups[Group].rampOn.switchOutletsLast) {
			...
			//Alles anschalten, wo Brighness und Power vorhanden
			await DeviceSwitchIfBriAndPower(adapter, Group, true);
			...
			adapter.RampOnIntervalObject[Group] = setInterval(async function () {
				...
				if (LoopCount >= RampSteps) {
					...
					//Alles anschalten, wo kein Brighness, jedoch Power vorhanden
					await DeviceSwitchIfNoBriAndPower(adapter, Group, true);
			...
			adapter.writeLog(
				`[ GroupPowerOnOff ] Anschalten mit Ramping und einfache Lampen zuerst für Group="${Group}`,
			);
		...
		//Ausschalten mit Ramping
			...
			adapter.RampOffIntervalObject[Group] = setInterval(async function () {
				...
				if (LoopCount >= RampSteps) {
					...
					await DeviceSwitchIfBriAndPower(adapter, Group, OnOff);
					await clearRampOffIntervals(adapter, Group);
				}
			}, Math.round(LightGroups[Group].rampOff.time / RampSteps) * 1000);
		}
		...
		adapter.writeLog(`[ v${adapter.version} GroupPowerOnOff ] error: ${e.message} / stack: ${e.stack}`, "error");
	}
}

/**
 * DeviceSwitchIfBriAndPower
 * @description Switch Power State, if Brightness and Power state available. Ausgelagert von GroupOnOff da im Interval kein await möglich
 * @param {object} adapter Adapter-Class
 * @param {string} Group Group of Lightgroups eg. LivingRoom, Children1,...
 * @param {boolean} OnOff true/false from power state
 */
async function DeviceSwitchIfBriAndPower(adapter, Group, OnOff) {
	try {
		adapter.writeLog(`[ DeviceSwitchIfBriAndPower ] Reaching for Group="${Group}, OnOff="${OnOff}"`, "info");
		...
		for (const Light of LightGroups[Group].lights) {
			if (Light?.bri?.oid) {
				...
				if (Light?.power?.oid) {
					await adapter.setForeignStateAsync(Light.power.oid, Light.power.onVal);
					adapter.writeLog(
						`[ DeviceSwitchIfBriAndPower ] Switching ${Light.description} (${Light.power.oid}) to: ${OnOff}`,
						"info",
					);
				} else {
					adapter.writeLog(
						`[ DeviceSwitchIfBriAndPower ] Can't switch to: ${OnOff}. No power & brightness state defined for Light = "${Light.description}" in Group = "${Group}"`,
						"warn",
					);
				}
			}
		...
		adapter.writeLog(`[ v${adapter.version} DeviceSwitchIfBriAndPower ] error: ${e.message} // stack: ${e.stack}`);
	}
}

/**
 * DeviceSwitchIfNoBriAndPower
 * @description Switch Power State, if Brightness not, but Power state available. Ausgelagert von GroupOnOff da im Interval kein await möglich
 * @param {object} adapter Adapter-Class
 * @param {string} Group Group of Lightgroups eg. LivingRoom, Children1,...
 * @param {boolean} OnOff true/false from power state
 */
async function DeviceSwitchIfNoBriAndPower(adapter, Group, OnOff) {
	try {
		adapter.writeLog(`[ DeviceSwitchIfNoBriAndPower ] Reaching for Group="${Group}, OnOff="${OnOff}"`, "info");
		...
		for (const Light of LightGroups[Group].lights) {
			if (!Light?.bri?.oid) {
				...
				if (Light?.power?.oid) {
					await adapter.setForeignStateAsync(Light.power.oid, Light.power.onVal);
					adapter.writeLog(
						`[ DeviceSwitchIfNoBriAndPower ] Switching ${Light.description} (${Light.power.oid}) to: ${OnOff}`,
						"info",
					);
				} else {
					adapter.writeLog(
						`[ DeviceSwitchIfNoBriAndPower ] Can't switch to: ${OnOff}. No power & brightness state defined for Light = "${Light.description}" in Group = "${Group}"`,
						"warn",
					);
				}
			}
		...
		adapter.writeLog(
			`[ v${adapter.version} DeviceSwitchIfNoBriAndPower ] error: ${e.message} // stack: ${e.stack}`,
		);
	}
}
