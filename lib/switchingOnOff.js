...
async function SimpleGroupPowerOnOff(adapter, Group, OnOff) {
	...
	try {
		if (!LightGroups[Group].lights.length) {
			adapter.writeLog(
				`[ SimpleGroupPowerOnOff ] Not able to switching ${operation} for Group = "${Group}". No lights are defined!!`,
				"warn",
			);
			return;
		}
		...
		const promises = LightGroups[Group].lights.map(async (Light) => {
			if (!Light?.power?.oid && !Light?.bri?.oid) {
				await adapter.writeLog(
					`[ SimpleGroupPowerOnOff ] Can't switching ${operation} for Light = "${
						Light.description
					}" in Group = "${Group}". No power or brightness state defined. Light: ${JSON.stringify(Light)}`,
					"warn",
				);
				return;
			}

			if (!Light?.bri?.useBri && Light?.power?.oid) {
				await Promise.all([
					adapter.setForeignStateAsync(Light.power.oid, OnOff ? Light.power.onVal : Light.power.offVal),
					adapter.writeLog(
						`[ SimpleGroupPowerOnOff ] Switching ${operation} ${Light.description} (${Light.power.oid}) with power state`,
					),
				]);
		...
		adapter.writeLog(
			`[ SimpleGroupPowerOnOff ] Simple power ${operation} without ramping for Group="${Group}"`,
			"info",
		);
...
async function GroupPowerCleaningLightOnOff(adapter, Group, OnOff) {
	...
	try {
		...
		if (OnOff) await SetBrightness(adapter, Group, 100);

		if (!OnOff) {
			const brightness = LightGroups[Group].adaptiveBri
				? await AdaptiveBri(adapter, Group)
				: adapter.LightGroups[Group].bri;
			await SetBrightness(adapter, Group, brightness);
			await adapter.setStateAsync(Group + ".power", false, false);
		}

		if (!LightGroups[Group].power && OnOff) {
