		...
		if (OnOff) {
			...
			for (const Light of LightGroups[Group].lights) {
				...
				if (!Light?.bri?.sendBri) {
					//Wenn sendBri nicht vorhanden oder false
					if (Light?.power?.oid) {
					...
					if (Light?.bri?.oid) {
						if (!LightGroups[Group].rampOn.enabled) {
							...
							if (LightGroups[Group].adaptiveBri) {
								...
								await SetBrightness(adapter, Group, adapter.LightGroups[Group].bri);
							}
						}
						adapter.writeLog(
							`[ SimpleGroupPowerOnOff ] Switching ${Light.description} (${Light.bri.oid}) to: ${OnOff}`,
						);
					} else {
						adapter.writeLog(
