	...
	try {
		...
		if (!LightGroups[Group].power) {
			...
			for (const Light of LightGroups[Group].lights) {
				if (!Light?.power?.oid) {
					adapter.writeLog(
						`Blink => Can't switch on. No power state defined for Light = "${Light.description}" in Group = "${Group}"`,
						"warn",
					);
				} else {
					await adapter.setForeignStateAsync(Light.power.oid, Light.power.onVal, false);
					adapter.log.debug(`Blink => Switching ${Light.description} ${Light.power.oid} to: on`);
				}
			}
			...
			if (
				...
				if (LightGroups[Group].power) {
					...
					for (const Light of LightGroups[Group].lights) {
						if (!Light?.power?.oid) {
							adapter.writeLog(
								`Blink => Can't switch off. No power state defined for Light = "${Light.description}" in Group = "${Group}"`,
								"warn",
							);
						} else {
							await adapter.setForeignStateAsync(Light.power.oid, Light.power.offVal, false);
							adapter.writeLog(
								`Blink: Switching ${Light.description} ${Light.power.oid} to: off`,
								"info",
							);
						}
					}
					...
					for (const Light of LightGroups[Group].lights) {
						if (!Light?.power?.oid) {
							...
							adapter.setForeignStateAsync(Light.power.oid, Light.power.onVal, false);
							adapter.writeLog(`Blink: Switching ${Light.description} ${Light.power.oid} to: on`, "info");
						}
					}
