...
async function SimpleGroupPowerOnOff(adapter, Group, OnOff) {
	try {
			...
			for (const Light of LightGroups[Group].lights) {
				...
				if (!Light?.bri?.useBri) {
					...
					if (Light?.power?.oid) {
						...
						adapter.writeLog(
							`[ SimpleGroupPowerOnOff ] Switching on ${Light.description} (${Light.power.oid})`,
						);
					} else {
						adapter.writeLog(
					...
					if (Light?.bri?.oid) {
						...
						adapter.writeLog(
							`[ SimpleGroupPowerOnOff ] Switching on ${Light.description} (${Light.bri.oid})`,
						);
					} else {
						adapter.writeLog(
			...
			for (const Light of LightGroups[Group].lights) {
				...
				if (!Light?.bri?.useBri) {
					...
					if (Light?.power?.oid) {
						...
						adapter.writeLog(
							`[ SimpleGroupPowerOnOff ] Switching off ${Light.description} (${Light.power.oid})`,
						);
					} else {
						adapter.writeLog(
					...
					if (Light?.bri?.oid) {
						...
						adapter.writeLog(
							`[ SimpleGroupPowerOnOff ] Switching off ${Light.description} (${Light.bri.oid})`,
						);
					} else {
						adapter.writeLog(
		...
		await adapter.SetLightState("SimpleGroupPowerOnOff");
		*/
		return true;
	} catch (error) {
		...
		if (OnOff && LightGroups[Group].rampOn.enabled && LightGroups[Group].rampOn.switchOutletsLast) {
			...
			for (const Light of LightGroups[Group].lights) {
				if (Light?.bri?.oid) {
					...
					if (Light?.power?.oid) {
						await adapter.setForeignStateAsync(Light.power.oid, Light.power.onVal);
						adapter.writeLog(
							`[ GroupPowerOnOff ] Switching ${Light.description} (${Light.power.oid}) on`,
							"info",
						);
					} else {
						adapter.writeLog(
							`[ GroupPowerOnOff ] Can't switch on. No power & brightness state defined for Light = "${Light.description}" in Group = "${Group}"`,
							"warn",
						);
					}
				}
			...
			for (const Light of LightGroups[Group].lights) {
				...
				if (Light?.power?.oid) {
					...
					adapter.writeLog(
						`[ GroupPowerOnOff ] Can't switch on. No power state defined for Light = "${Light.description}" in Group = "${Group}"`,
						"warn",
					);
				}
			}
				...
				if (LoopCount >= RampSteps) {
					...
					for (const Light of LightGroups[Group].lights) {
						...
						if (Light?.power?.oid) {
							await adapter.setForeignStateAsync(Light.power.oid, Light.power.offVal);
							adapter.writeLog(
								`[ GroupPowerOnOff ] Switching ${Light.description} (${Light.power.oid}) off`,
								"info",
							);
						} else {
							adapter.writeLog(
								`[ GroupPowerOnOff ] Can't switch off. No power state defined for Light = "${Light.description}" in Group = "${Group}"`,
								"warn",
							);
						}
					}
			...
			for (const Light of LightGroups[Group].lights) {
				if (!Light?.bri?.oid) {
					...
					if (Light?.power?.oid) {
						...
						adapter.writeLog(
							`[ GroupPowerOnOff ] Can't switch off. No power & brightness state defined for Light = "${Light.description}" in Group = "${Group}"`,
							"warn",
						);
					}
				}
...
async function DeviceSwitch(adapter, Group, OnOff) {
	try {
		adapter.writeLog(`[ DeviceSwitch ] Reaching for Group="${Group}, OnOff="${OnOff}"`, "info");
...
async function AutoOnLux(adapter, Group) {
	try {
		...
		adapter.errorHandling(error, "AutoOnLux");
	}
}
...
async function AutoOnMotion(adapter, Group) {
	try {
		...
		adapter.errorHandling(error, "AutoOnMotion");
	}
}
