...
const { clearAutoOffTimeouts, clearRampIntervals, clearTransitionTimeout, clearBlinkIntervals } = require("./timers");
const {
	setDeviceBri,
...
const { SetValueToObject, limitNumber } = require("./helper");
...
async function GroupPowerOnOff(adapter, Group, OnOff) {
	const LightGroups = adapter.LightGroups;
	try {
		if (!LightGroups[Group] || !LightGroups[Group].rampOn?.enabled || !LightGroups[Group].rampOff?.enabled) {
			adapter.writeLog(
				`[ GroupPowerOnOff ] No rampOn or rampOff datapoints available for Group = "${Group}". Please check your config and restart the adapter!!`,
				"warn",
			);
			return;
		}
 ...
 */
async function RampWithInterval(adapter, Group, rampUp = true) {
	const LightGroups = adapter.LightGroups;
	const Lights = LightGroups[Group]?.lights || [];
	const RampSteps = adapter.GlobalSettings.RampSteps;
	const RampTime = await limitNumber(LightGroups[Group].rampOn.time, 10);
	let LoopCount = 0;
	try {
		adapter.RampIntervalObject[Group] = adapter.setInterval(async function () {
			LoopCount++;
			...
			if (LoopCount >= RampSteps || !promises.length) {
				await clearRampIntervals(adapter, Group);
				return true;
			}
		}, Math.round(RampTime / RampSteps) * 1000);
	} catch (error) {
		adapter.errorHandling(error, "RampWithInterval");
	}
}
...
async function TurnOnWithRamping(adapter, Group) {
	...
	try {
		...
		if (LightGroups[Group]?.rampOn?.enabled && LightGroups[Group].rampOn?.switchOutletsLast) {
			adapter.writeLog(`[ ${funcName} ] Switch off with ramping and simple lamps last for Group="${Group}"`);

			await BrightnessDevicesSwitchPower(adapter, LightGroups[Group].lights, true); // Turn on lights for ramping is no use Bri is used
			await RampWithInterval(adapter, Group, true); // Returns true if finished or no lights with ramping without transition time
			...
			adapter.writeLog(`[ ${funcName} ] Anschalten mit Ramping und einfache Lampen zuerst für Group="${Group}`);
	...
	try {
		...
		if (OnOff) {
			if (LightGroups[Group].power) {
				await Promise.all([SetBrightness(adapter, Group, 100), SetCt(adapter, Group, 6500)]);
			...
			if (!LightGroups[Group].lastPower) {
				LightGroups[Group].power = false;
				await SimpleGroupPowerOnOff(adapter, Group, false);
			}
		}
