		...
		if (OnOff) {
			...
			for (const Light in LightGroups[Group].lights) {
				if (!LightGroups[Group].lights[Light].power) {
					adapter.writeLog(
						`SimpleGroupPowerOnOff => Can't switch on. No power state defined for Light = "${LightGroups[Group].lights[Light].description}" in Group = "${Group}"`,
						"info",
					);
				} else {
					...
					adapter.writeLog(
						`SimpleGroupPowerOnOff => Switching ${Light} ${LightGroups[Group].lights[Light].power.oid} to: ${OnOff}`,
					);
				}
			}
		} else {
			...
			for (const Light in LightGroups[Group].lights) {
				if (!LightGroups[Group].lights[Light].power) {
					adapter.writeLog(
						`SimpleGroupPowerOnOff => Can't switch off. No power state defined for Light = "${LightGroups[Group].lights[Light].description}" in Group = "${Group}"`,
						"info",
					);
				} else {
					await adapter.setForeignStateAsync(
...
async function blink(adapter, Group) {
	try {
		...
		if (!LightGroups[Group].power) {
			...
			adapter.writeLog(`on ${loopcount}`, "info");
