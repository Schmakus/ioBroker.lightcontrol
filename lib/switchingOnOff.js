 ...
 * @param {object} adapter Adapter-Class
 */
async function RampWithInterval(adapter, Group) {
	const LightGroups = adapter.LightGroups;
	...
	let LoopCount = 0;
	try {
		adapter.RampOnIntervalObject[Group] = adapter.setInterval(async function () {
			LoopCount++;
...
async function TurnOnWithRamping(adapter, Group) {
	...
	try {
		if (LightGroups[Group]?.rampOn?.enabled && LightGroups[Group].rampOn?.switchOutletsLast) {
			...
			await Promise.all([clearRampOnIntervals(adapter, Group), clearRampOffIntervals(adapter, Group)]);
			...
			await RampWithInterval(adapter, Group); // Returns true if finished or no lights with ramping without transition time
