...
async function SimpleGroupPowerOnOff(adapter, Group, OnOff) {
	...
	try {
		...
		await Promise.all([useBrightness, outlast, useSwitch]);
		return true;
	} catch (error) {
...
async function GroupPowerOnOff(adapter, Group, OnOff) {
	...
	try {
		if (!LightGroups[Group] || LightGroups[Group]?.rampOn?.enabled === undefined || LightGroups[Group]?.rampOff?.enabled === undefined) {
			adapter.writeLog(
